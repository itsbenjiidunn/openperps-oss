...
| `SetRiskConfig` | Market authority | Sets three per-market risk knobs in the `[RISK_CFG_SEED, market]` PDA (each 0 = disabled), enforced in `PlaceOrder` / `PlaceBatchOrder`: a dynamic OI multiplier (bounds the vault's net position per asset at `vault_equity * mult / 10_000`, base units at the live mark, layered on `SetHouseCap`, the tighter wins); a per-wallet position cap (max net base position any one wallet may hold per asset); and a stale-pause (block new risk-increasing trades once the mark is un-refreshed past N slots, de-risking always allowed). |
