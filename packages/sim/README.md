...
on-chain mechanics cannot: **does the House -- and so its LPs -- actually survive
...
- The House is the counterparty to every user trade (B-book); its net position
  per asset is the user imbalance, bounded by `SetHouseCap`.
- **Income: skew funding only.** The House is always the thin side, so it always
...
- **Cost 1: inventory mark-to-market** (net short a pumping token, realized when
  the longs close at the top), bounded by the House cap.
- **Cost 2: the trading fee.** The engine charges both sides and routes it to
  insurance, so the House *pays* its leg's fee; it does not earn it.

The `feeToHouseBps` knob models a counterfactual the engine does **not** implement
(the House earning a maker fee/spread), to quantify how much that would change
viability.

## The finding (run `report` for the numbers)

Under the current engine, on one-sided memecoin flow the House is **not a yield
role -- it is a bounded-loss backstop.** Funding is negligible, the fee is a cost
to the House, balanced churn bleeds that fee, and a fast pump-and-exit realizes a
real (cap-bounded) loss. The House cap is the load-bearing protection.

The lever that turns volume into LP yield is **routing a maker fee TO the House**
(a real spread for taking the unpopular side) -- an engine/wrapper change, not a
parameter tweak. Until then: size the House cap to a loss you can fund, treat the
House as a customer-acquisition cost, and monetize at the App/integrator layer
